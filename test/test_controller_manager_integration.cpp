  ...
  bool waitForControllerManagerNode(double timeout_seconds = TEST_TIMEOUT_S)
  {
    ...
    while (rclcpp::ok()) {
      ...
      if (elapsed > timeout_seconds) {
        RCLCPP_ERROR(node_->get_logger(), "Timeout: controller_manager node not found within %.1f seconds", timeout_seconds);
        return false;
      }
...
TEST_F(ControllerManagerIntegrationTestFixture, ControllerManagerNodeExists)
{
  ...
  EXPECT_TRUE(waitForControllerManagerNode()) 
    << "controller_manager node should be running in the system";
}
